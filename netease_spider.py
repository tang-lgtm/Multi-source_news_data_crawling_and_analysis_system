...
class NeteaseSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # 定位文章列表区域
        articles = response.css('div.data_row.news_article.clearfix ')

        for article in articles:
            item = NewsCrawlItem()

            # 提取文章标题
            item['title'] = article.css('div.news_title h3 a::text').get()

            # 提取文章 URL
            item['url'] = article.css('div.news_title h3 a::attr(href)').get()

            # 提取发布时间（列表页中的"小时前"）
            item['publish_time_relative'] = article.css('div.news_tag span.time::text').get()
    ...
    def parse_article(self, response):
        ...
        # 提取具体的发布时间（详情页中的 YYYY-MM-DD HH:MM:SS 格式）
        time_string = response.css('div.post_info::text').get()
        match = re.search(r'(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})', time_string)
        if match:
            item['publish_time'] = match.group(1)
        else:
        ...
        # 提取文章内容
        item['content'] = ' '.join(response.css('div.post_body p::text').getall()).strip()
